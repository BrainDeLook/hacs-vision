...
> ⚠️ **HACS Vision 未收录进 HACS 默认商店**，无法在商店里直接搜索安装。请通过下方「自定义仓库」手动添加本仓库 URL。
...
### v6.5.1 (2026-07-16) — 翻译结果本地持久化
- **优化**: **翻译结果本地缓存** — 翻译成功后的 README 会存入浏览器 `localStorage`（默认保留 7 天，单条上限 1MB、最多 50 条 LRU 淘汰）。关闭窗口、刷新页面或重启 HA 后再次打开弹窗，命中缓存即**瞬间显示、不再调用大模型**，零耗时零 token 消耗
- **附带收益**: 已翻译过的仓库即便在设置中未配置翻译 AI，也能直接查看历史翻译结果（缓存命中无需后端请求，自然规避了第三方仓库 GitHub Token 404 的问题）

### v6.5 (2026-07-16) — README AI 翻译（稳定版）& 交互优化
- **新增**: **README AI 翻译** — 仓库弹窗内新增语言切换条（原文 / 中文 / 英文 / 德文，可选 日文 / 韩文），调用你在 HA「设置 → 语音助手」中已配置的对话助手（如 MiMo / GPT / Ollama）就地翻译说明文档，无需任何额外 API 密钥或硬编码地址
- **新增**: **翻译引擎可配置** — 设置页「基本设置」新增「README 翻译 AI」下拉，自动枚举系统内所有 `conversation.*` 实体，由你自由选择翻译所用的助手
- **新增**: **翻译缓存** — 同一仓库同一目标语言的翻译结果服务端缓存 6 小时，减少重复调用
- **优化**: **未配置翻译 AI 时隐藏按键** — 若在设置中未选择「README 翻译 AI」，仓库弹窗不再显示翻译语言条，改为提示「请先在设置中选择 README 翻译 AI」，避免看到按键却翻不出来的误会
- **稳定性**: **基于 6.4.4 稳定内核** — 移除会导致翻译调用失败的超时参数，回归稳定可用的翻译链路
